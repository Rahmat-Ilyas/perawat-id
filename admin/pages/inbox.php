<?php
$data = mysqli_query($conn, "SELECT * FROM tb_message ORDER BY id DESC");
...
$data1 = mysqli_query($conn, "SELECT * FROM tb_message ORDER BY id DESC LIMIT $first, $page");

$_SESSION['inbox'] = "text-danger";
$_SESSION['send_inbox'] = "";
...
<form method="POST" action="controller.php">
	<div class="" role="toolbar">
		<div class="btn-group">
			...
			<button type="submit" name="hapus_pesan" class="btn btn-light waves-effect" data-toggle1="tooltip" title="Hapus Pilihan"><i class="mdi mdi-delete font-18 vertical-middle"></i></button>
			<a href="<?= url('inbox') ?>" role="button" class="btn btn-light waves-effect" data-toggle1="tooltip" title="Segarkan"><i class="mdi mdi-refresh font-18 vertical-middle"></i></a>
		</div>
	</div>
	<h4 class="mt-4">Pesan Masuk</h4>
	<div class="border mt-3">
		<div class="mt-0">
			<div class="">
				<ul class="message-list">
					<?php foreach ($data1 as $dta) { ?>
						<li class="<?= $dta['status'] ?>">
							<a href="<?= url('read_inbox').'&id='.$dta['id'] ?>">
								<div class="col-mail col-mail-1">
									<div class="checkbox-wrapper-mail">
										<input type="checkbox" class="chk" name="chk[]" value="<?= $dta['id'] ?>" id="chk<?= $dta['id'] ?>">
										<label for="chk<?= $dta['id'] ?>" class="toggle"></label>
									</div>
									<p class="title" style="margin-left: -40px;"><?= $dta['nama']." (".$dta['email'].")" ?></p>
								</div>
		...
		<div class="row m-3">
			...
			<div class="col-5">
				<div class="btn-group float-right">
					...
					if ($back < 1) $back = 1;
					?>
					<a href="<?= url('inbox')."&page_inbox=".$back ?>" data-toggle1="tooltip" title="Back"><button type="button" class="btn btn-gradient waves-effect" <?php if ($thispage == 1) echo "disabled"; ?> ><i class="fa fa-chevron-left"></i></button></a>
					<a href="<?= url('inbox')."&page_inbox=".$next ?>"  data-toggle1="tooltip" title="Next"><button type="button" class="btn btn-gradient waves-effect" <?php if ($thispage == $totalpage) echo "disabled"; ?> ><i class="fa fa-chevron-right"></i></button></a>
				</div>
			</div>
		</div>
	</div>
